...
public interface BundleMapper {
    ...
    //
    // @Insert("insert into bundle (\"namePackage\", \"zipCodeOrigin\", \"zipCodeDestin\", \"description\", \"status\")
    // values(#{namePackage}, #{zipCodeOrigin}, #{zipCodeDestin}, #{description}, #{status}) ")
    // @Options(useGeneratedKeys = true, keyProperty = "id")
    // boolean insert(Bundle bundle);
    //
    // @Update("update bundle set \"namePackage\" = #{namePackage}, \"zipCodeOrigin\" = #{zipCodeOrigin},
    // \"zipCodeDestin\" = #{zipCodeDestin}, \"description\" = #{description}, \"status\" = #{status} where \"id\" =
    // #{id} ")
